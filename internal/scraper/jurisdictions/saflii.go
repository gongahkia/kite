...
// SAFLIIScraper implements scraping for SAFLII (Southern African Legal Information Institute)
type SAFLIIScraper struct {
	*scraper.BaseScraper
	baseURL string
	client  *http.Client
}

// NewSAFLIIScraper creates a new SAFLII scraper
func NewSAFLIIScraper() *SAFLIIScraper {
	baseURL := "https://www.saflii.org"
...
func (ss *SAFLIIScraper) SearchCases(ctx context.Context, query scraper.SearchQuery) ([]*models.Case, error) {
	searchURL, err := ss.buildSearchURL(query)
	if err != nil {
		return nil, errors.ParsingError("failed to build search URL", err)
	}

	allowed, err := ss.BaseScraper.client.CheckRobots(ctx, "/cgi-bin/sinosrch.cgi")
	if err != nil || !allowed {
		return nil, errors.ErrRobotsDisallowed
	}

	if err := ss.BaseScraper.client.rateLimiter.Wait(ctx); err != nil {
	...
	req.Header.Set("User-Agent", ss.BaseScraper.client.userAgent)
	...
	doc.Find("li").Each(func(i int, s *goquery.Selection) {
		...
		if s.Find("a").Length() > 0 {
			caseData := ss.extractCaseFromSearchResult(s)
			if caseData != nil {
...
func (ss *SAFLIIScraper) GetCaseByID(ctx context.Context, caseID string) (*models.Case, error) {
	caseURL := ss.buildCaseURL(caseID)

	allowed, err := ss.BaseScraper.client.CheckRobots(ctx, "/")
	if err != nil || !allowed {
		return nil, errors.ErrRobotsDisallowed
	}
	...
	return ss.extractCaseDetails(doc, caseID, caseURL)
}

// GetCasesByDateRange retrieves cases within a date range
func (ss *SAFLIIScraper) GetCasesByDateRange(ctx context.Context, startDate, endDate time.Time, limit int) ([]*models.Case, error) {
	query := scraper.SearchQuery{
		StartDate: &startDate,
		EndDate:   &endDate,
		Limit:     limit,
	}
	return ss.SearchCases(ctx, query)
}

// IsAvailable checks if SAFLII is available
func (ss *SAFLIIScraper) IsAvailable(ctx context.Context) bool {
	req, err := http.NewRequestWithContext(ctx, "HEAD", ss.baseURL, nil)
	if err != nil {
		return false
	}
	resp, err := ss.client.Do(req)
...
func (ss *SAFLIIScraper) buildSearchURL(query scraper.SearchQuery) (string, error) {
	params := url.Values{}
	if query.Query != "" {
	...
	searchURL := fmt.Sprintf("%s/cgi-bin/sinosrch.cgi?%s", ss.baseURL, params.Encode())
	return searchURL, nil
}

// buildCaseURL builds a case URL from a case ID
func (ss *SAFLIIScraper) buildCaseURL(caseID string) string {
	caseID = strings.TrimSpace(caseID)
	if strings.Contains(caseID, "/") {
		if !strings.HasPrefix(caseID, "http") {
			return fmt.Sprintf("%s%s", ss.baseURL, caseID)
		}
		return caseID
	}
	return fmt.Sprintf("%s/za/cases/%s.html", ss.baseURL, caseID)
}

// extractCaseFromSearchResult extracts case data from a search result item
func (ss *SAFLIIScraper) extractCaseFromSearchResult(s *goquery.Selection) *models.Case {
	c := models.NewCase()
	...
	if strings.Contains(c.URL, "/ZACC/") {
		c.Court = "Constitutional Court of South Africa"
	} else if strings.Contains(c.URL, "/ZASCA/") {
		c.Court = "Supreme Court of Appeal of South Africa"
	} else if strings.Contains(c.URL, "/ZAGPPHC/") {
		c.Court = "Gauteng Division, Pretoria (High Court)"
	} else if strings.Contains(c.URL, "/ZAWCHC/") {
		c.Court = "Western Cape Division, Cape Town (High Court)"
	}

	c.Jurisdiction = "South Africa"
	c.SourceDatabase = "SAFLII"
...
func (ss *SAFLIIScraper) extractCaseDetails(doc *goquery.Document, caseID, caseURL string) (*models.Case, error) {
	c := models.NewCase()
	c.ID = caseID
	c.URL = caseURL
	...
	// Extract court from URL
	if strings.Contains(caseURL, "/ZACC/") {
		c.Court = "Constitutional Court of South Africa"
	} else if strings.Contains(caseURL, "/ZASCA/") {
		c.Court = "Supreme Court of Appeal of South Africa"
	} else if strings.Contains(caseURL, "/ZAGPPHC/") {
		c.Court = "Gauteng Division, Pretoria (High Court)"
	} else if strings.Contains(caseURL, "/ZAWCHC/") {
		c.Court = "Western Cape Division, Cape Town (High Court)"
	}
