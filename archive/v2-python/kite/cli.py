"""
Command-line interface for Kite library.
"""
...
def search_command(args) -> None:
    ...
    try:
        with scraper_class() as scraper:
            logger.info("searching", scraper=args.scraper, query=args.query)
            ...
            logger.info("search_completed", cases_count=len(cases))
...
def get_case_command(args) -> None:
    ...
    try:
        with scraper_class() as scraper:
            logger.info(
                "getting_case", case_id=args.case_id, scraper=args.scraper
            )
            ...
            logger.info("case_retrieval_completed")

    except Exception as e:
        logger.error(
            "case_retrieval_failed", error=str(e), error_type=type(e).__name__
        )
        print(f"Error: {str(e)}", file=sys.stderr)
        sys.exit(1)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="kite",
        description="Kite - Legal case law scraper",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  kite search courtlistener "privacy rights" --limit 10
  kite search canlii "charter rights" --start-date 2023-01-01
  kite get-case courtlistener "12345"
  kite list-scrapers
        """,
    )

    parser.add_argument(
        "--version", action="version", version="Kite 1.0.0"
    )
    parser.add_argument(
